...
avg_price_age_group = df.groupby(['Age Group'])['Total Price'].mean().reset_index(name='Avg Price')

fig3 = px.bar(avg_price_age_group, x='Age Group', y='Avg Price', color='Avg Price',
              color_continuous_scale="peach", category_orders={'Age Group': age_labels},
...
fig3.update_traces(hovertemplate="Age Group: %{x}<br>Average Spending: %{y:$,.2f}")
fig3.add_hline(y=overall_avg_price, line_dash="dash", line_color="#1CBE4F",
               annotation_text=f'Overall Average: ${overall_avg_price:.2f}',
               annotation_font_color="#1CBE4F", annotation_position="top left")
fig3.update_layout(title={'x': 0.5, 'y': 0.9}, width=700, height=450, xaxis_title=None, yaxis_title='Average Spending ($)', yaxis=dict(range=[500, 800], tick0=500, dtick=50), showlegend=False)
fig3.show()

age_group_churned_returned = df.groupby('Age Group').agg(
    total_churned=('Churn', 'sum'),
    churn_rates=('Churn', 'mean'),
    total_returned=('Returns', 'sum'),
    return_rates=('Returns', 'mean')
...
fig4 = px.bar(age_group_churned_returned, x='Age Group', y=['total_churned', 'total_returned'], barmode='group',
              category_orders={'Age Group': age_labels},
              title='4. Customer Churn and Returns by Age Group', text_auto='.3s',
              labels={'value': 'Number of Customers'}, hover_data={'churn_rates': True, 'return_rates': True})
fig4.update_traces(hovertemplate="Age Group: %{x}<br>Total: %{y:,.0f}<br>Churn Rate: %{customdata[0]}%<br>Return Rate: %{customdata[1]}%<extra></extra>")
...
for index, row in age_group_churned_returned.iterrows():
    fig4.add_annotation(x=row['Age Group'], y=row['total_churned'],
                        text=f"{row['churn_rates']:.1f}%", showarrow=False, xanchor='right', yanchor='bottom',
                        font=dict(color='blue', size=11))
    fig4.add_annotation(x=row['Age Group'], y=row['total_returned'],
                        text=f"{row['return_rates']:.1f}%", showarrow=False, xanchor='left', yanchor='bottom',
                        font=dict(color='red', size=11))

fig4.update_layout(title={'x': 0.5, 'y': 0.9}, width=800, height=500, bargap=0.1, xaxis_title=None, legend_title=None, legend=dict(orientation="h", yanchor="bottom", y=1.02, xanchor="center", x=0.5))
fig4.show()
...
def train_and_test(X, y, classifier, test_size, enable_print=True): 
    ...
    # Calculate training accuracy and confusion matrix
    ...
    # Calculate testing accuracy and confusion matrix
...
# Prepare X and y
X = df.drop(columns=["Churn", "Purchase Date"])  
y = df["Churn"]
test_size = 0.2
...
models = {
    ...
    "Decision Tree": DecisionTreeClassifier(random_state=42, max_depth=10, min_samples_split=10, min_samples_leaf=5, max_leaf_nodes=100),
    "XGBoost": XGBClassifier()
}

# Train and test each model, and store their accuracy
model_accuracies = {}
trained_models = {}

for model_name, model in models.items():
    print(f"========================== {model_name} ==========================")
    accuracy, trained_model = train_and_test(X, y, model, test_size)
